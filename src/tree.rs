...
impl<const LEAF_FANOUT: usize> Tree<LEAF_FANOUT> {
    ...
    /// # let config = melange_db::Config::tmp().unwrap();
    /// # let db: melange_db::Db<1024> = config.open()?;
    /// db.insert(&[0], vec![0])?;
    /// assert_eq!(db.get(&[0]).unwrap(), Some(melange_db::InlineArray::from(vec![0])));
    ...
    /// assert_eq!(db.insert(&[1, 2, 3], vec![1]).unwrap(), Some(melange_db::InlineArray::from(&[0])));
    /// # Ok(()) }
    /// ```
    #[doc(alias = "set")]
    #[doc(alias = "put")]
    ...
    /// # let db: melange_db::Db<1024> = config.open()?;
    /// db.insert(&[1], vec![1]);
    /// assert_eq!(db.remove(&[1]).unwrap(), Some(melange_db::InlineArray::from(vec![1])));
    ...
    /// ```
    /// # fn main() -> Result<(), Box<dyn std::error::Error>> {
    /// use melange_db::{Config, InlineArray};
    ///
    ...
    /// # let config = melange_db::Config::new().path("batch_doctest");
    /// # let db: melange_db::Db<1024> = config.open()?;
...
/// ```
/// # fn main() -> Result<(), Box<dyn std::error::Error>> {
/// use melange_db::{Batch, open};
///
/// # let _ = std::fs::remove_dir_all("batch_db_2");
/// let db: melange_db::Db<1024> = open("batch_db_2")?;
/// db.insert("key_0", "val_0")?;
///
