...
### ⚠️ 并发限制说明
> **重要提示**: Melange DB 当前使用的 EBR (Epoch-Based Reclamation) 库在高并发场景下存在已知限制。

**问题描述**:
- 在多个线程同时进行大量并发操作时，EBR 库可能触发 `RefCell already borrowed` panic
- 这是一个底层库的限制，不影响原子性操作的正确性
- 生成的数据仍然保证唯一性和一致性

**触发条件**:
- 多线程同时调用 `update_and_fetch`、`compare_and_swap` 等原子操作
- 高频率的并发读写操作
...
- **原子性操作和序列表**: `cargo run --example simple_atomic_sequence`
  - 演示如何使用 `update_and_fetch` 实现自增序列
  - 展示类似 MongoDB 的序列表功能
  - 包含并发测试验证原子性
